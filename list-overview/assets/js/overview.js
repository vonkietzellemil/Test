...
const dbPromise = new Promise((resolve, reject) => {
  const request = indexedDB.open("ListApp", 1);
...
async function getRowImages(rowId) {
  const db = await dbPromise;
...
async function deleteImagesByRowId(rowId) {
  const imgs = await getRowImages(rowId);

  await Promise.all(
    imgs.map(img => deleteImage(img.id))
  );
}
...
async function openEditModal(item) {
  ...
  if (item.type === "list" || item.type === "category") {
    ...
    const modal = createModal(modalTitle, modalContent, modalBtns);
    ...
    imagePreviewContainer.addEventListener("click", async e => {
      ...
      if (deleteBtn) {
        const confirmed = confirm('Remove image?');
        if (!confirmed) return;

        await deleteImage(
          deleteBtn.dataset.imageId
        );

        deleteBtn.closest(".image-wrapper").remove();
        return;
      }

      const img = e.target.closest(".image-thumb");
    ...
    if (galleryInput) {
      galleryInput.addEventListener("change", async () => {
        ...
        const images = await getRowImages(item.id);
    ...
    if (addImageBtn) {
      addImageBtn.addEventListener("click", openImageSourceSelector);
    }
  }
